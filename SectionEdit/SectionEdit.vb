Public Class SectionEdit
    ...
    Private Sub SectionEdit_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        If Command() = String.Empty Then
            Me.Close()
        End If
        If Not Mid(Command(), 3, 2) = "PH" Then
            MsgBox("Invalid group, please use this plugin for sections only")
            Me.Close()
        End If

        Dim Split As String() = Command.Split(" - ")

        SectionID = Mid(Split(0), 5)
        If Mid(Command, 1, 4) = "ENPH" Then
            Try
retry:
                Dim FS As New IO.FileStream(Split(2), IO.FileMode.Open)
                ...
                Point = Readkmp(buffer2, Readkmp(buffer2, &H18, 4) + &H54 + (&H10 * SectionID), &H10)
            Catch iox As IO.IOException
                Dim box As MsgBoxResult = MsgBox(iox.Message, MsgBoxStyle.RetryCancel)
...
retry2:
                ...
                Point = Readkmp(buffer2, Readkmp(buffer2, &H20, 4) + &H54 + (&H10 * SectionID), &H10)
            Catch iox As IO.IOException
                Dim box As MsgBoxResult = MsgBox(iox.Message, MsgBoxStyle.RetryCancel)
                If box = MsgBoxResult.Retry Then
                    GoTo retry2
                ...
                End If
            End Try
        ElseIf Mid(Command, 1, 4) = "CKPH" Then
            Try
retry3:
                Dim FS As New IO.FileStream(Split(2), IO.FileMode.Open)
                Dim BR As New IO.BinaryReader(FS)
                ...
                Point = Readkmp(buffer2, Readkmp(buffer2, &H28, 4) + &H54 + (&H10 * SectionID), &H10)
            Catch iox As IO.IOException
                Dim box As MsgBoxResult = MsgBox(iox.Message, MsgBoxStyle.RetryCancel)
                If box = MsgBoxResult.Retry Then
                    GoTo retry3
